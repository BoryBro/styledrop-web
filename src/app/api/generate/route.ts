...
const STYLE_PROMPTS: Record<string, Record<string, string>> = {
  ...
  "joseon-farmer": {
    ...
    // v2 — 레퍼런스 이미지 1의 전체 스타일(분위기, 색감, 구도, 포즈)을 최대한 그대로 따름
    "v2": "Transform this photo into a Joseon Dynasty Korean farmer portrait, matching the exact visual style of the reference image as closely as possible.\n\nSTYLE REFERENCE — FOLLOW PRECISELY:\n- Replicate the exact composition, pose, framing, and crop of the reference image\n- Match the color grading, sepia tone, contrast, brightness, and film grain of the reference\n- Match the scene setting, background, and lighting quality of the reference\n- The final result should look as if it belongs in the same photo collection as the reference\n\nFACE — PRESERVE IDENTITY, INTEGRATE NATURALLY:\n- Preserve the person's facial features: bone structure, eye shape, nose, lips, proportions\n- Same expression and gaze direction\n- CRITICAL: The face must feel naturally integrated — same lighting, same contrast, same film grain as the rest of the image\n- The face must NOT look like it was pasted onto a different body\n- Apply the same sepia/aged tone uniformly across face and body\n- No beautification, no modern skin smoothing\n- Add surface weathering matching the reference's aged quality\n\nHAIR — MUST CHANGE COMPLETELY:\n- Replace with authentic Joseon-era farmer hair as shown in the reference\n- Unwashed, coarse, unstyled\n\nCLOTHING:\n- Traditional Korean hemp work clothing (삼베옷) as seen in the reference\n- Worn, dirty fabric matching the reference's texture and aging\n\nPHOTOGRAPH QUALITY:\n- Match the reference image's photographic quality exactly — grain, softness, tone, degradation",

    // v3 — 레퍼런스 이미지 2의 전체 스타일(분위기, 색감, 구도, 포즈)을 최대한 그대로 따름
    "v3": "Transform this photo into a Joseon Dynasty Korean farmer portrait, matching the exact visual style of the reference image as closely as possible.\n\nSTYLE REFERENCE — FOLLOW PRECISELY:\n- Replicate the exact composition, pose, framing, and crop of the reference image\n- Match the color grading, sepia tone, contrast, brightness, and film grain of the reference\n- Match the scene setting, background, and lighting quality of the reference\n- The final result should look as if it belongs in the same photo collection as the reference\n\nFACE — PRESERVE IDENTITY, INTEGRATE NATURALLY:\n- Preserve the person's facial features: bone structure, eye shape, nose, lips, proportions\n- Same expression and gaze direction\n- CRITICAL: The face must feel naturally integrated — same lighting, same contrast, same film grain as the rest of the image\n- The face must NOT look like it was pasted onto a different body\n- Apply the same sepia/aged tone uniformly across face and body\n- No beautification, no modern skin smoothing\n- Add surface weathering matching the reference's aged quality\n\nHAIR — MUST CHANGE COMPLETELY:\n- Replace with authentic Joseon-era farmer hair as shown in the reference\n- Unwashed, coarse, unstyled\n\nCLOTHING:\n- Traditional Korean hemp work clothing (삼베옷) as seen in the reference\n- Worn, dirty fabric matching the reference's texture and aging\n\nPHOTOGRAPH QUALITY:\n- Match the reference image's photographic quality exactly — grain, softness, tone, degradation",
  },
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // joseon-farmer 레퍼런스: Image2 장면을 그대로 유지하고 얼굴만 Image1로 교체
    const isJoseonRef = style === "joseon-farmer" && refCount > 0;
    const promptText = refCount === 0
      ? `Edit this image: ${prompt}`
      : isJoseonRef
        ? `Image 1 contains the source person's face. Image 2 is the target photograph — keep everything in Image 2 exactly as-is (pose, body, clothing, background, lighting, sepia tone, film grain, all photographic qualities). Your only task is to replace the face in Image 2 with the face of the person in Image 1. The replaced face must blend seamlessly into Image 2 — match the exact sepia tone, film grain, contrast, lighting direction, and skin weathering of Image 2. The face must feel like it was always part of this historical photograph, not pasted on. Do not alter anything else in Image 2.`
